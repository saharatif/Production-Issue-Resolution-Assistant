...
class SimulationServiceTest(unittest.TestCase):
    ...
    def test_generate_scenario_batch_temp_spike_targets_line_b(self):
        batch = generate_scenario_batch("temp_spike")
        line_b = next(reading for reading in batch if reading["line_id"] == "LINE-B")

        self.assertEqual(line_b["machine_status"], "CRITICAL_TEMPERATURE_SPIKE")
        self.assertGreaterEqual(line_b["temperature_c"], 280.0)
        self.assertLessEqual(line_b["temperature_c"], 320.0)

    def test_generate_scenario_batch_data_gap_targets_line_b(self):
        batch = generate_scenario_batch("data_gap")
        line_b = next(reading for reading in batch if reading["line_id"] == "LINE-B")

        self.assertEqual(line_b["machine_status"], "DATA_GAP")
        self.assertIsNone(line_b["temperature_c"])
        self.assertIsNone(line_b["pressure_bar"])
        self.assertIsNone(line_b["vibration_mm_s"])
        self.assertIsNone(line_b["output_units_per_hour"])

    def test_generate_scenario_batch_does_not_mutate_other_lines(self):
        for scenario in ("pressure_drop", "temp_spike", "data_gap"):
            batch = generate_scenario_batch(scenario)
            other_lines = [r for r in batch if r["line_id"] != "LINE-B"]
            for reading in other_lines:
                self.assertEqual(
                    reading["machine_status"],
                    "RUNNING",
                    f"Non-LINE-B reading had unexpected status in scenario '{scenario}'",
                )
